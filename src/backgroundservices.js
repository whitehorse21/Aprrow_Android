    ...
    async startsyncingservice() {
        ...
        BackgroundTimer.runBackgroundTimer(async () => {
            ...
            if (status != null && status == "true")
                return;
            // console.log("syncing service running");
            //console.log("background syncing is watching");
            try {
                ...
                if (isconnected) {
                    ...
                    //  alert("widgets>>>>>"+request_obj.widget.length+"  devices>>>>>"+ request_obj.device.length);
                    if (request_obj.widget.length > 0 || request_obj.device.length > 0) {
                        ...
                        var aws_lamda = require("./config/AWSLamdaConfig.json");
                        var acceestoken = await commons.get_token();
                        const response = await fetch('' + aws_data.path + aws_data.stage + aws_lamda.bulkupdate, {
                            method: 'POST',
                            headers: {
                                Accept: 'application/json',
                                'Content-Type': 'application/json',
