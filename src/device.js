...
import { Text, TextInput, Image, View, StyleSheet, ScrollView, TouchableOpacity,ToastAndroid, Button, FlatList,AsyncStorage } from 'react-native';
...
export default class device extends Component {
   ...
   async updateLocaldb()
   {
       ...
       if(connectionstatus)
       {
        ...
        // console.log("maxtime=============>>>>"+JSON.stringify(maxtime));
         var deviceListObj = await databasehelper.getAlldeviceDeletedAlso();
        // console.log("deviceListObj===>>>>"+JSON.stringify(deviceListObj));
         ...
         fetch('' + aws_data.path + aws_data.stage + aws_lamda.userdatamgnt, {
          ...
          .then(async (responseJson) => {
           ...
           var updatedeviceArray=[];
            ...
            //console.log("updatedeviceArray===>>>>"+JSON.stringify(updatedeviceArray));
           // console.log("newdeviceArray===>>>>"+JSON.stringify(newdeviceArray));              
    ...
    async rename_click() {
        ...
        if (this.state.device_name.length <= 15) {
            ...
            if (result.results.rowsAffected > 0) {
                //dname[this.state.flag]=this.state.device_name;
                this.device_display();
                //this.setState({ dialogVisible_rename: false });
            }
    ...
    async welcomebox() {
        ...
        /*  var user=await databasehelper.getuser();
          var username=user.res.rowObj.username;
